...
class SerotoCourse(models.Model):
  ...
  landing_page_url = fields.Char(
    string='Trang landing',
    help='Đường dẫn trang giới thiệu chi tiết của khóa học trên website (vd: '
         '/khoa-hoc). Dùng để hiển thị nút "Chi tiết" cạnh nút "Đăng ký ngay" ở '
         'snippet Course Card - để trống nếu khóa học chưa có trang riêng.',
  )
  ...
  def action_create_landing_page(self):
    ...
    seroto_form.page_maukhoahoc - xem ghi chú chi tiết trong
    ...
    template_view = self.env.ref('seroto_form.page_maukhoahoc_view')
    course_type_labels = dict(self._fields['course_type'].selection)
    new_key = 'seroto_form.page_course_%s' % self.id
    ...
    arch = arch.replace('t-name="seroto_form.page_maukhoahoc"', 't-name="%s"' % new_key)
